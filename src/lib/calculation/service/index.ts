...
class DefaultCalculationService implements ICalculationService {
  ...
  getRealm(worldType: string, level: number): RealmResult {
    ...
    const { getRealmByLevel, getNextRealm, getNextRealmLevel, getRealmMultiplier } = require('../../data/realmData');
    
    const realmName = getRealmByLevel(worldType, level);
    const nextRealm = getNextRealm(worldType, level);
    const nextRealmLevel = getNextRealmLevel(worldType, level);
    const multiplier = getRealmMultiplier(worldType, level);
