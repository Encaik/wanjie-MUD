 ...
 * 使用说明：
 * - 按 WorldType（世界类型）分类
 * - 每个世界类型有统一的术语键，但值根据世界观不同而变化
 * - 术语用于统一替换游戏中的文本，使不同世界有不同风味
 * 
 * 扩展方式：
 * 1. 在对应世界类型的对象中修改术语值
 * 2. 确保所有世界类型的术语键保持一致
 ...
 * 
 * 命名规范：
 * - power: 力量来源名称（如灵力、真气、魔力）
 * - energy: 能量源名称（如灵气、魔法元素）
 * - resource: 通用货币/资源名称（如灵石、魔晶）
 * - treasure: 宝物统称（如法宝、神器）
 * - core: 修炼核心（如丹田、魔力核心）
 * - enemy: 敌人统称（如妖兽、魔兽）
 * - dungeon: 秘境统称（如秘境、地下城）
 */
 ...
 * - 术语键名保持一致，值根据世界观变化
 */
export const WORLD_TERMINOLOGY: Record<WorldType, WorldTerminology> = {
  '修仙': {
    power: '灵力',
    energy: '灵气',
    practice: '修炼',
...
export function getTerminology(worldType: WorldType): WorldTerminology {
  return WORLD_TERMINOLOGY[worldType] || WORLD_TERMINOLOGY['修仙'];
}
...
export function getDungeonInfo(worldType: WorldType): { name: string; desc: string; location: string } {
  ...
  return {
    name: terms.dungeon,
    desc: terms.dungeonDesc,
    location: terms.dungeonLocation,
  };
}
