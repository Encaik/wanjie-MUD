...
describe('itemManager', () => {
  describe('createItemInstance', () => {
    it('从模板创建物品实例', () => {
      const inst = createItemInstance('wanjie:common:spirit_stone', { quantity: 100 });
      expect(inst.templateId).toBe('wanjie:common:spirit_stone');
  ...
  describe('addItem', () => {
    it('添加可堆叠物品自动合并', () => {
      const inv = addItem([], 'wanjie:common:spirit_stone', 500);
      const inv2 = addItem(inv, 'wanjie:common:spirit_stone', 300);
      expect(getItemCount(inv2, 'wanjie:common:spirit_stone')).toBe(800);
      // 只应该有一个实例
      expect(inv2.filter(i => i.templateId === 'wanjie:common:spirit_stone').length).toBe(1);
    });
    ...
    it('quantity=0 不添加', () => {
      const inv = addItem([], 'wanjie:common:spirit_stone', 0);
  ...
  describe('removeItem', () => {
    it('移除部分数量', () => {
      let inv = addItem([], 'wanjie:common:spirit_stone', 500);
      const instanceId = inv[0].instanceId;
      inv = removeItem(inv, instanceId, 200);
      expect(getItemCount(inv, 'wanjie:common:spirit_stone')).toBe(300);
    });
    ...
    it('移除全部数量则删除实例', () => {
      ...
      expect(getItemCount(inv, 'wanjie:common:spirit_stone')).toBe(0);
      expect(inv.length).toBe(0);
    });
  });
  ...
  describe('hasEnough', () => {
    it('数量足够返回 true', () => {
      const inv = addItem([], 'wanjie:common:spirit_stone', 1000);
      expect(hasEnough(inv, 'wanjie:common:spirit_stone', 500)).toBe(true);
    });

    it('数量不足返回 false', () => {
      const inv = addItem([], 'wanjie:common:spirit_stone', 100);
      expect(hasEnough(inv, 'wanjie:common:spirit_stone', 500)).toBe(false);
    });
  });

  describe('getCurrencyAmount', () => {
    it('获取灵石余额（语法糖）', () => {
      const inv = addItem([], 'wanjie:common:spirit_stone', 888);
      expect(getCurrencyAmount(inv, 'wanjie:common:spirit_stone')).toBe(888);
    });
  });
  ...
  describe('resolveItem', () => {
    ...
    it('解析货币实例', () => {
      const inst = createItemInstance('wanjie:common:spirit_stone', { quantity: 500 });
      const resolved = resolveItem(inst);
