...
function registerWsEndpoint(path: string): WebSocketServer {
  console.log(`[WS] Registering endpoint: ${path}`);
...
function handleUpgrade(req: IncomingMessage, socket: Duplex, head: Buffer) {
  ...
  if (wss) {
    console.log(`[WS] Found handler for ${pathname}, upgrading...`);
    wss.handleUpgrade(req, socket, head, (ws) => {
      console.log(`[WS] Connection upgraded for ${pathname}`);
      wss.emit('connection', ws, req);
    });
  } else if (!dev) {
    // 生产环境销毁未注册的 upgrade 请求，防止连接泄漏
    // 开发环境不销毁 —— Next.js HMR 需要通过 /_next/webpack-hmr 建立 WebSocket
    console.log(`[WS] No handler for ${pathname}, destroying connection`);
    socket.destroy();
  } else {
    console.log(`[WS] No handler for ${pathname} (dev mode, skipping)`);
  }
}
...
setupGameHandler(registerWsEndpoint('/ws/game'));
...
app.prepare().then(() => {
  ...
  server.listen(PORT, () => {
    ...
    console.log(`> WebSocket endpoints: ${Array.from(wssMap.keys()).join(', ')}`);
  });
});
