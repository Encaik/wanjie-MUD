# registry-driven-world-stats

## Purpose

世界数值配置（baseHp、hpPerLevel、attackPerLevel 等）完全由 `WorldTypeData` 注册提供，消费代码无硬编码兜底。`getWorldData()` 函数零内联数值常量。
...
- **THEN** SHALL 包含 `stats` 对象，字段包括：`baseHp`、`hpPerLevel`、`hpPerConstitution`、`baseAttack`、`attackPerLevel`、`attackPerConstitution`、`attackPerSpiritRoot`、`baseDefense`、`defensePerLevel`、`defensePerWillpower`、`enemyAttackBonus`、`enemyDefenseBonus`、`statDisplayNames`
- **AND** 所有数值字段 SHALL 为 `number` 类型
