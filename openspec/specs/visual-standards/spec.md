...
All interactive elements (buttons, links, selectable cards) SHALL provide visual feedback for hover, focus, active, and disabled states. Hover and active states SHALL use lift + shadow transitions rather than scale transforms.

#### Scenario: Button states
- **WHEN** a button is hovered
- **THEN** it lifts with `translateY(-1px)` and enhanced shadow
...
Components SHALL use component-type-specific `rounded-*` values instead of uniform rounding.
