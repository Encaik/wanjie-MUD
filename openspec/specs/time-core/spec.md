...
统一时间系统核心能力，包含游戏世界时钟（GameClock）、现实时钟（RealClock）、冷却管理、时间格式化和服务端时间获取。所有功能以纯函数方式实现，位于 `core/time/`，不依赖 React。
...
- **THEN** `gameClock.format(time)` 返回 `"初元3年七月十五 巳时 (18岁)"`

#### Scenario: 简短格式显示
- **WHEN** 调用 `gameClock.formatShort(time)`
- **THEN** 返回 `"第3年7月15日 巳时 · 18岁"`
