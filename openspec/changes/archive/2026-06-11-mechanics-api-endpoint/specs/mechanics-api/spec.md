...
- **THEN** 服务端 SHALL 返回 `404 Not Found`，错误信息为 `世界观 'nonexistent' 未注册`
...
前端组件 SHALL 通过 HTTP API 获取世界机制信息，SHALL NOT 直接调用 `WorldMechanicsRegistry` 或 `getWorldMechanics()` 函数。`factory.ts` 中的 `getWorldMechanics()` 和 `hasUniqueMechanics()` 导出 SHALL 被删除。
...
- **THEN** SHALL 通过 `worldview.mechanics != null` 判断
- **AND** SHALL NOT 调用 `hasUniqueMechanics()`
