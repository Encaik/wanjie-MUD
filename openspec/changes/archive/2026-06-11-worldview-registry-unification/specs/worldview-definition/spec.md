...
世界观定义 SHALL 通过 Mod 系统的 JSON 文件加载到 `WorldViewRegistry`。每个世界观的 JSON 文件 SHALL 包含 `WorldviewDefinition` 所需的所有字段。加载过程 SHALL 发生在服务端初始化阶段（`app/api/init.ts`）。

Mod 的 content type 为 `'worldview'`（替代旧的 `'world'`）。
...
- **AND** 注册完成后 `WorldViewRegistry.getInstance().count` SHALL 等于已加载的世界观数量
...
`WorldviewDefinition` SHALL 完全替代旧的 `WorldTypeData` 接口。`WorldTypeData` 接口 SHALL 被删除。所有引用 SHALL 使用 `WorldviewDefinition`。

#### Scenario: WorldTypeData 不再存在于类型定义中

- **WHEN** 在所有源代码中搜索 `WorldTypeData`
- **THEN** SHALL NOT 存在该标识符的定义或引用
- **AND** 旧引用 SHALL 已替换为 `WorldviewDefinition`

#### Scenario: 相关函数同步重命名

- **WHEN** 调用获取世界观数据的方法
- **THEN** 使用 `WorldViewRegistry.getInstance().get(id)` 而非 `getWorldTypeData(id)`
- **AND** 使用 `WorldViewRegistry.getInstance().getAll()` 而非 `getAllWorldTypeValues()`
- **AND** 使用 `WorldViewRegistry.getInstance().register(view)` 而非 `registerWorldType(def)`
...
- **THEN** SHALL 通过 `WorldViewRegistry.getInstance().get(world.worldviewId)` 获取 `WorldviewDefinition`
