...
`WorldViewRegistry`（存储 `WorldviewDefinition` 实例的 Map）在 `src/core/registry/WorldViewRegistry.ts` 中 SHALL 作为所有世界观数据的唯一定义来源。任何其他文件 SHALL NOT 重复定义世界名称前缀、后缀、描述、系数、世界观文本或特性池。所有消费代码 SHALL 通过 `WorldViewRegistry.getInstance().get(worldviewId)` 获取数据。
...
- **THEN** SHALL 通过 `WorldViewRegistry.getInstance().get(worldviewId).texts` 获取
...
- **WHEN** 比较 `registry.get('cultivation')` 和 `registry.get('xianxia')`
...
- **WHEN** 读取 `WorldViewRegistry.getInstance().get('cultivation')` 返回的 `WorldviewDefinition`
- **THEN** `stats.baseHp`、`stats.hpPerLevel` 等数值字段 SHALL 等于注册数据中 `stats` 对象的对应字段
- **AND** SHALL NOT 存在从消费代码内联的数值常量

#### Scenario: 缺失任何数值字段时报错

- **WHEN** 一个世界观的注册数据缺少 `stats` 或其子字段
- **THEN** 系统 SHALL 抛出错误，明确指出缺失字段和世界观 ID
...
- **THEN** registry 的验证逻辑 SHALL 在注册时抛出错误
